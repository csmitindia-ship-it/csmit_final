...
const ManageAccommodationPage: React.FC = () => {
  ...
  const handleVerifyBooking = async (bookingId: number) => {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/accommodation/bookings/${bookingId}/verify`, {
            method: 'PUT',
        });
        if (!response.ok) {
        ...
        if (err instanceof Error) {
            ...
            setError('An unknown error occurred while verifying.');
        }
    } finally {
        setVerifyingId(null);
    }
                ...
                <table className="min-w-full bg-gray-800 border border-gray-700">
                    <thead>
                        <tr>
                            ...
                            <th className="py-2 px-4 border-b border-gray-700">Verification</th>
                                ...
                                <td className="py-2 px-4 border-b border-gray-700">
                                    {!booking.isVerified && (
                                        <button
                                            ...
                                            className="bg-green-600 hover:bg-green-700 text-white font-bold py-1 px-3 rounded disabled:bg-gray-400"
                                            disabled={verifyingId === booking.id}
                                        >
                                            {verifyingId === booking.id ? 'Verifying...' : 'Verify'}
                                        </button>
                                    )}
                                </td>
                            </tr>
